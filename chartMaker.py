...
def getChart(upc, storeNum, periodLen, periodStr):
	...
	for i in range(periodLen):
		t = threading.Thread(target=getSalesPerDay, args=(periodData,allData,i)) # use concurrency
		t.start()
	dayStrings = list(map(toStr,map(getKeys, periodData))) #convert from datetime to string
	if(periodStr=="week"):
		dayStrings = list(map(cutYear, dayStrings)) # don't show year in strings for week graphs
	dayStrings = dayStrings[::-1] # reverse (from oldest to newest)
	quantities = list(map(getValue, periodData))
	quantities = quantities[::-1] # reverse
	plt.ylim(min(quantities)-3 if min(quantities)-3>0 else 0, max(quantities)+3) # set y axis limit to min - 3 and max + 3 
	line=plt.plot(dayStrings, quantities)
	xticks = plt.gca().xaxis.get_major_ticks()

	if periodStr == "month" or periodStr == "year":
		for i in range(periodLen):
			if(i!=0 and i!= (periodLen//5) and i!= 2*(periodLen//5) and 
			i!= 3*(periodLen//5) and i!= 4*(periodLen//5) and i!=periodLen-1): #only show 6 ticks for month and year
